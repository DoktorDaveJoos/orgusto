<template>
  <div class="flex flex-col p-4">
    <span class="uppercase font-medium text-xs text-gray-800 leading-tight">{{ __('common.free_tables') }}</span>
    ...
    handleTableClick(tableId) {
      if (this.selectedTables.find(table => table.id === tableId)) {
        ...
        this.$emit('value:changed', {
          tables: this.selectedTables.concat(this.tables.find(table => table.id === tableId)),
        });
      }
    },
    isActive(tableId) {
