...
<script>
export default {
    props: ["filterData", "tablesEndpoint", "error", "init"],
    data() {
        return {
            tables: this.init ? this.init : [],
            chosenTables: this.init ? this.init : [],
        };
    },
    mounted() {
    ...
    watch: {
        ...
        tables: function (n, o) {
            // merge free tables with initial tables
            n = _.union(n, o);

            // if (this.init) {
            //   this.init.forEach((table) => {
            //     const found = n.find((elem) => elem === table);
            //     if (found === undefined) {
            //       n.push(table);
            //     }
            //   });
