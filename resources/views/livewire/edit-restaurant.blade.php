<div>
    ...
    <div class="w-full flex justify-center mt-6">
        <div class="max-w-5xl flex-1 flex text-blue-600 hover:text-blue-800 items-center">
                <svg class="h-3 mr-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd"
                          d="M9.707 16.707a1 1 0 01-1.414 0l-6-6a1 1 0 010-1.414l6-6a1 1 0 011.414 1.414L5.414 9H17a1 1 0 110 2H5.414l4.293 4.293a1 1 0 010 1.414z"
                          clip-rule="evenodd"/>
                </svg>
                <a class="text-xs" href="/restaurants">Zurück zu Restaurants</a>
        </div>
    </div>

    @infocard(['title' => $name])
    @slot('optional_button')
        <button type="button" @if (!$is_dirty) disabled @endif wire:click="submit"
    ...
    <div class="flex flex-col sm:flex-row flex-wrap p-6 border-b border-gray-200">
        ...
        <input id="name" wire:model.debounce.500ms="name"
               class="orgusto-input w-full sm:w-64 flex-1 transition-all duration-150 ease-in-out"
        ...
        <input id="email" wire:model.debounce.500ms="contact_email"
               class="orgusto-input w-full sm:w-64 transition-all duration-150 ease-in-out"
               placeholder="{{ __('restaurants.email') }}"/>
            ...
            <tr class="hover:bg-gray-100">
                ...
                <td class="px-6">
                    <div x-data="{handle:'{{ $user->type}}' }" class="text-xs leading-5">
                        <span class="font-semibold rounded-full -ml-2 px-2 py-1"
                              x-bind:class="handle === 'registered' ? 'text-green-800 bg-green-200' : 'text-gray-800 bg-gray-200'">{{ $user->type }}</span>
                    </div>
                </td>
                ...
                <td class="px-6 py-4 text-right text-sm leading-5 font-medium">
                    @if ($user->type == 'anonymous' || $user->type == 'invited' || $user->id == auth()->user()->id)
                        <a class="orgusto-button text-indigo-600 bg-indigo-100 hover:text-white hover:bg-indigo-600 transition-colors duration-150 ease-in-out"
                           href="{{ route('user.show', ['user' => $user]) }}">{{ __('restaurants.edit') }}</a>
                    @endif
                </td>
                <td class="px-6 py-4 flex justify-end">
                    @if($user->pivot->role != 'admin')
                        <button wire:click="removeAccount({{ $user->id }})"
                                class="orgusto-button my-auto mr-0 bg-red-100 text-red-600 hover:text-white hover:bg-red-600 transition-colors duration-150 ease-in-out">{{ __('restaurants.remove') }}</button>
                    @endif
                </td>
            </tr>
        @endforeach
    ...
    @slot('table_body')
        @foreach($tables as $table)
            <tr class="hover:bg-gray-100">
                ...
                <td class="px-6 py-4 text-right text-sm leading-5 font-medium">
                    <a class="orgusto-button text-indigo-600 bg-indigo-100 hover:text-white hover:bg-indigo-600 transition-colors duration-150 ease-in-out"
                       href="{{ route('restaurant.table.show', ['restaurant' => $restaurant, 'table' => $table]) }}">{{ __('restaurants.edit') }}</a>
                </td>
                <td class="px-6 py-4">
                    <button wire:click="deleteTable({{ $table->id }})"
                            class="orgusto-button my-auto mr-0 bg-red-100 text-red-600 hover:text-white hover:bg-red-600 transition-colors duration-150 ease-in-out">{{ __('restaurants.delete') }}</button>
                </td>
            </tr>
        @endforeach
    @endslot
