...
@if (auth()->user()->selected &&!auth()->user()->selected->subscribed())

    <div x-data="{ open: true }" x-show="open" class="bg-indigo-600 z-50">
              ...
              <span class="flex p-2 rounded-lg bg-indigo-800">
                <svg class="h-6 w-6 text-white" x-description="Heroicon name: outline/speakerphone"
                  ...
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <span>
                        Du hast für dein Restaurant {{ auth()->user()->selected->name }} noch kein Abonnement.
                    </span>
                    </p>
                </div>
                <div class="order-3 mt-2 flex-shrink-0 w-full sm:order-2 sm:mt-0 sm:w-auto">
                    <a href="/billing/restaurant/{{ auth()->user()->selected->id }}"
